 ...
 * @author grosbois
 */
public class TrackInfoFrame extends JInternalFrame implements
  InternalFrameListener, PropertyChangeListener, ActionListener,
  TrackModificationListener {
  ...
  private static final Color transparentBlue = new Color(0f, 0f, 1.0f, 0.5f),
    transparentWhite = new Color(1f, 1f, 1.0f, 0.5f),
   ...
   * Constructeur.
   *
   * @param bundle contient les données du trajet.
   */
  public TrackInfoFrame(HashMap<String, Object> bundle) {
    ...
    correctedElevations = new JCheckBoxMenuItem(resB.
      getString("altitude_correction"));
    ...
    elevLbl = new JLabel("<html><i><small>" + emptyTxt + "<br>" + emptyTxt
      + "</small></i></html>");
    ...
    denivLbl = new JLabel("<html><i><small>" + emptyTxt + "<br>" + emptyTxt
      + "</small></i></html>");
    ...
    durationLbl = new JLabel("<html><i><small>" + emptyTxt + "<br>" + emptyTxt
      + "</small></i></html>");
    ...
    speedLbl = new JLabel("<html><i><small>" + emptyTxt + "<br>" + emptyTxt
      + "</small></i></html>");
   ...
   *
   * @param bundle Données de la trace ou null pour effacer une trace existante.
   *
   */
  ...
  private void updateTrack() {
    ...
    if (correctedElevations.isSelected()) {
      ...
      parseAndDisplaySubTrack(-1, -1); // analyser et afficher la trace entière
    }
  }
...
//          }
//        }
//        // Moyenne glissante de la vitesse avec suppression des extrema
//        float moyenne = 0;
//        for (int j = 0; j < NVAL; j++) {
//          if (j != imin && j != imax) {
//            moyenne += v[j];
...
//        }
//        moyenne /= (NVAL - 2);
//        g.speed = moyenne; // remplacer la vitesse instantanée par la moyenne
  ...
  private void parseAndDisplaySubTrack(int start, int end) {
    ...
    distanceLbl.setText("<html>" + dist2String(subpathCumulDist)
      + "</html>");
    elevLbl.setText("<html><i><small>"
      + elev2HTML(subpathElevMin, subpathElevMax)
      + "</small></i></html>");
    denivLbl.setText("<html><i><small>"
      + deniv2HTML(denivCumulPos, denivCumulNeg) + "<small></i></html>");
    durationLbl.setText("<html><i><small>"
      + time2String(dureeTotale, true) + "<br>(" + time2String(dureeSansPause, true)
      + ")"
      + "</small></i></html>");
    speedLbl.setText("<html><i><small>"
  ...
  public void clear() {
    ...
    denivLbl.setText("<html><i><small>" + emptyTxt + "<br>" + emptyTxt
      + "</small></i></html>");
    elevLbl.setText("<html><i><small>" + emptyTxt + "<br>" + emptyTxt
      + "</small></i></html>");
    durationLbl.setText("<html><i><small>" + emptyTxt + "<br>" + emptyTxt
      + "</small></i></html>");
    speedLbl.setText("<html><i><small>" + emptyTxt + "<br>" + emptyTxt
      + "</small></i></html>");
  }
  ...
  public void actionPerformed(ActionEvent e) {
    if (correctedElevations.isSelected()) { // Tâche de correction d'altitude
      corr = new ElevationCorrection();
      corr.addPropertyChangeListener(this);
      corr.execute();
  ...
  /**
   * Recherche des altitudes ramenées au niveau du sol dans un Thread séparé car
   * les requêtes IGN peuvent prendre du temps. Ces nouvelles altitudes sont
   * récupérées par Internet auprès de l'IGN qui les calcule à partir d'un
   * modèle de terrain.
   */
  private class ElevationCorrection extends SwingWorker<Void, Void> {
    ...
    /**
     * Récupérer les groupements d'altitudes corrigées auprès de l'IGN tout en
     * mettant à jour la barre de progression de la fenêtre.
     *
     * @return
     * @throws Exception
     */
    ...
    protected Void doInBackground() throws Exception {
      ...
      // Identifier le nombre d'altitudes à corriger et repérer les indices
    ...
    /**
     * Appeler les méthodes de mise à jour les champs d'altitude, de distance et
     * de vitesse une fois les corrections terminées. La barre de progession
    ...
    /**
     * Les altitudes ne peuvent être récupérées que par groupe de 20 maximum.
     *
     * @param latitude
     * @param longitude
     * @param elevation
     * @param nbLoc
     */
    private void quickGetGeoNamesAsterElevations(double[] latitude, double[] longitude,
     ...
     *
     * @param latitude tableau de latitudes des géolocalisations à corriger
     * @param longitude tableau de longitudes des géolocalisations à corriger
     * @param elevation tableau pour stocker les altitudes corrigées
     * @param nbLoc taille utile des tableaux
     */
    private void getQuickIGNElevations(double[] latitude, double[] longitude,
      ...
      try {
        ...
        StringTokenizer st = new StringTokenizer(
          reponse.substring(startAltIdx + 1, endAltIdx), ",");
        int j = 0;
     ...
     *
     * <p>
     * Trois modèles d'élévation (DEM: Digital Elevation Model) possibles:<ul>
    ...
    private float getAsterElevation(double latitude, double longitude) {
      ...
      try {
//      url = new URL("http://api.geonames.org/gtopo30?lat="+latitude+
...
//              "&lng="+longitude+"&username=rgrosbois"); // SRTM3
        url = new URL("http://api.geonames.org/astergdem?lat=" + latitude
          + "&lng=" + longitude + "&username=rgrosbois"); // Aster Global
   ...
   * pause en km/h.
   *
   * @param distanceTot Distance totale de la trace en mètres.
   * @param dureeTotale Durée de la trace avec les pauses.
   * @param dureeSansPause Durée de la trace sans les pause.
  ...
  private String meanSpeeds2HTML(float distanceTot, long dureeTotale,
    ...
    if (dureeTotale == 0) {
      ...
      return String.format(Locale.getDefault(), "%.1fkm/h<br>(%.1fkm/h)", moyenne,
        moyenne2);
    }
  }
   ...
   *
   * @param denivCumulPos Dénivelé positif en mètres
   * @param denivCumulNeg Dénivelé négatif en mètres
   *
   ...
   *
   * @param pathElevMin Altitude minimale
   * @param pathElevMax Altitude maximale
   * @return chaîne au format HTML
   */
  private String elev2HTML(int pathElevMin, int pathElevMax) {
    return String.format(Locale.getDefault(), "%dm /%dm<br>(%dm)",
      pathElevMin, pathElevMax, pathElevMax - pathElevMin);
  }
  ...
  class MixedGraph extends JComponent {
     ...
     * non.
     *
     * @param mx Abscisse informatique de la souris
     * @param my Ordonnée informatique de la souris
     * @return
     */
    private boolean isInsideGraph(int mx, int my) {
      return mx >= (xMin + XSEP) && mx <= (xMax - XSEP)
        && my >= (yMin + YSEP) && my <= (yMax - YSEP);
    }
    ...
    /**
     * Déterminer la géolocalisation la plus proche de l'endroit où se situe la
     * souris.
     *
     * @param e
     ...
     *
     * @param c Coordonnées informatiques
     * @param d Distance en mètres
    ...
    protected void paintComponent(Graphics gp) {
      ...
      Graphics2D g2 = (Graphics2D) gp; // récupérer l'outil graphique Swing

      // Copie de l'outil graphique pour les légendes
      Graphics2D g2Legends;
      g2Legends = (Graphics2D) g2.create();
      final float dash1[] = {2f};
      g2Legends.setStroke(new BasicStroke(1.0f, BasicStroke.CAP_BUTT,
        BasicStroke.JOIN_MITER, 2f, dash1, 0.0f));
      ...
      bounds = g2Legends.getFontMetrics().getStringBounds(tmpS, g2Legends);
      xMax = width - ((int) bounds.getWidth() + BASE_MARGIN); // Marge droite
      ...
      // | Courbe d'altitude |
      // +-------------------+
      // Créer la courbe tout en identifiant l'ordonnée de l'altitude maximale 
      // servant au dégradé
      ...
      g2ElevShape.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
      ...
      g2ElevFill.setPaint(new GradientPaint(0, (int) yElevMax, transparentWhite, 0,
        yMax, transparentRed));
      ...
      g2SpeedFill.setPaint(new GradientPaint(0, 0, transparentBlue, 0, height,
        transparentWhite));
      ...
      int incr;
      if (deniv < 1000) {
        incr = 100; // si la plage est inférieure à 1000 m => graduer tous les 100 m 
      } else {
      ...
      if (deltaV < 20) {
        incr = 2; // si la plage est inférieure à 20 km/h => graduer tous les 2 km/h 
      } else {
      ...
      for (int i = (int) graphSpeedMin; i <= graphSpeedMax; i += incr) {
        ...
        bounds = g2Legends.getFontMetrics().getStringBounds(tmpS, g2Legends);
        g2Legends.drawString(tmpS, xMax + INNER_SEP,
          (float) (c.y + bounds.getHeight() / 2));
        // Axe intermédiaire
      ...
      for (int d = 0; d <= distTot; d += incr) {
        ...
        g2Legends.draw(new Line2D.Float(c.x, yMin + YSEP,
          c.x, yMax + INNER_SEP / 2));
        // Légende
      ...
      if (dureeTot < 60 * 30) { // Si moins de 30mn => graduer toutes les 5mn
        incr = 5 * 60;
      } else if (dureeTot < 60 * 60) {// Si moins d'1h => graduer toutes les 10mn
        incr = 10 * 60;
      } else if (dureeTot < 60 * 60 * 2) { // Si moins de 2h => graduer tous les 1/4h
        incr = 15 * 60;
      ...
      for (int t = incr; t < dureeTot; t += incr) {
        ...
        g2Legends.draw(new Line2D.Float(c.x, yMin - INNER_SEP / 2,
          c.x, yMax - YSEP));
        // Légende
      ...
      if (firstGeoLocIdx != -1) {
        ...
        if (secondGeoLocIdx != -1) { // Intervalle de géolocalisations
          ...
          if (cEnd.x > c.x) {
            ...
            g2interv.fill(new Rectangle2D.Float(cEnd.x, yMin, c.x - cEnd.x,
              yMax - yMin));
          }
        }
        ...
        Rectangle2D rect = new Rectangle2D.Double(c.x - bounds.getWidth() / 2 - INNER_SEP,
          yMax - bounds.getHeight() - INNER_SEP,
          bounds.getWidth() + 2 * INNER_SEP, bounds.getHeight() + INNER_SEP);
        g2Legends.fill(rect);
        g2g.draw(rect);
        g2g.drawString(tmpS,
          (float) (c.x - bounds.getWidth() / 2), yMax - INNER_SEP);
        // Durée courante
        tmpS = time2String(g.timeStampS - g0.timeStampS, true);
        bounds = g2g.getFontMetrics().getStringBounds(tmpS, g2g);
        rect = new Rectangle2D.Double(c.x - bounds.getWidth() / 2 - INNER_SEP,
          yMin, bounds.getWidth() + 2 * INNER_SEP, bounds.getHeight() + INNER_SEP);
        g2Legends.fill(rect);
        g2g.draw(rect);
        g2g.drawString(tmpS, (float) (c.x - bounds.getWidth() / 2),
          (float) (yMin + bounds.getHeight()));
        // Altitude
        tmpS = ((int) g.dispElevation) + "m";
        bounds = g2ElevShape.getFontMetrics().getStringBounds(tmpS, g2ElevShape);
        rect = new Rectangle2D.Double(c.x - bounds.getWidth() / 2 - INNER_SEP,
          c.y, bounds.getWidth() + 2 * INNER_SEP, bounds.getHeight() + INNER_SEP);
        g2Legends.fill(rect);
        g2ElevShape.draw(rect);
        g2ElevShape.drawString(tmpS, (float) (c.x - bounds.getWidth() / 2),
  ...
  public interface GeoLocSelectionListener {
    ...
    /**
     * Prévient les écouteurs qu'un sous-ensemble de la trace a été sélectionné.
     *
     * @param geoIdx1 Index de la géolocalisation de départ.
     * @param geoIdx2 Index de la géolocalisation d'arrivée.
     */
